...
/**
 * @title InitiaGameLogger
 * @dev Contract for logging game results on Initia EVM Testnet
 * Supports Roulette, Mines, Wheel, and Plinko games
 * Pyth Entropy is used on Arbitrum Sepolia for randomness
 */
contract InitiaGameLogger is Ownable {
     ...
     * @param betAmount Bet amount in wei (INIT)
     * @param resultData Encoded game result data
     * @param payout Payout amount in wei (INIT)
    ...
    function logGameResult(
        ...
        emit GameResultLogged(
            logId, player, gameType, betAmount, payout,
            entropyRequestId, entropyTxHash, block.timestamp
        );
    ...
    function getPlayerHistory(address player, uint256 limit) external view returns (bytes32[] memory) {
        ...
        if (limit == 0 || limit >= logs.length) return logs;
