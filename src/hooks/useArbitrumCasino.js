...
const formatEthAmount = (amount) => {
  // Use 18 decimals for Ethereum/CreditCoin
  return (parseFloat(amount) / 1000000000000000000).toFixed(8);
};
...
export const useArbitrumCasino = () => {
  ...
  const placeRouletteBet = useCallback(async (betType, betValue, amount, numbers = []) => {
    ...
    try {
      ...
      const payload = CasinoGames.roulette.placeBet(betType, betValue, parseMonAmount(amount), numbers);
  ...
  const startMinesGame = useCallback(async (betAmount, minesCount, tilesToReveal) => {
    ...
    try {
      ...
      const payload = CasinoGames.mines.startGame(parseMonAmount(betAmount), minesCount, tilesToReveal);
  ...
  const spinWheel = useCallback(async (betAmount, segments) => {
    ...
    try {
      ...
      const payload = CasinoGames.wheel.spin(parseMonAmount(betAmount), segments);
