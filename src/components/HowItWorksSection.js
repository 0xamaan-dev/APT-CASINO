...
const HowItWorksSection = () => {
  ...
  const steps = [
    ...
    {
      id: 2,
      title: 'Get INIT tokens',
      description: 'Power your gameplay with INIT tokens of Initia EVM Testnet Network.',
      emoji: '💰'
    },
    {
      id: 3,
    ...
    },
    {
      id: 4,
      title: 'Earn Rewards',
      description: 'Win INIT tokens and unlock exclusive perks through our multi-tiered loyalty program. Earn cashback on losses and gain access to tournaments.',
      emoji: '🏆'
    },
  ];
          ...
          <div className="p-[1px] bg-gradient-to-r from-red-magic via-purple-500 to-blue-magic rounded-xl shadow-xl">
            <div className="bg-[#1A0015]/70 backdrop-blur-sm rounded-xl p-5">
              {steps.map((step) => (
                <div
                  key={step.id}
                  className={`mb-4 p-4 rounded-lg cursor-pointer transition-all duration-300 transform ${activeStep === step.id
                ...
                >
                  <div className="flex items-start">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-4 shadow-lg transform transition-all duration-300 ${activeStep === step.id
            ...
            <div className="relative w-full max-w-xl h-[400px]">
              ...
              <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 flex space-x-2">
                {steps.map((step) => (
                  <button
                    key={step.id}
                    className={`w-6 h-2 rounded-full transition-all duration-300 ${activeStep === step.id
