...
const RouletteWithHistory = () => {
  ...
  const [betConfig, setBetConfig] = useState({
    betType: 'straight',
    betValue: 7,
    betAmount: ethers.parseEther('0.01') // 0.01 INIT
  });
  ...
  const playGame = async () => {
    ...
    try {
      ...
      // NOTE: When integrated with CreditCoinGameLogger service, the initiaTxHash
      // would be captured from the logGameResult response and stored here:
      // const gameLogResult = await initiaGameLogger.logGameResult({...});
      // setGameState(prev => ({ ...prev, initiaTxHash: gameLogResult.initiaTxHash }));
        ...
        <div className="bg-white rounded-lg border p-6 mb-6">
          ...
          <div className={`p-4 rounded-lg text-center ${gameState.result.isWin ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'
            ...
            <div className="text-sm text-gray-600">
              Bet: {ethers.formatEther(betConfig.betAmount)} INIT →
              Payout: {ethers.formatEther(gameState.result.payoutResult.payout.toString())} INIT
            </div>
            <div className={`font-medium ${gameState.result.isWin ? 'text-green-600' : 'text-red-600'
              }`}>
              Profit: {gameState.result.isWin ? '+' : ''}{ethers.formatEther(gameState.result.payoutResult.profit.toString())} INIT
            </div>
          </div>
        </div>
      )}
      ...
      {gameState.vrfDetails && (
        <div className="bg-white rounded-lg border p-6 mb-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-gray-900">Cross-Chain Verification</h3>
          ...
          <div className="space-y-4">
            {/* Arbitrum Sepolia Entropy Transaction */}
            <div className="p-3 bg-purple-50 rounded-lg border border-purple-200">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-xs font-semibold text-purple-700">PYTH ENTROPY</span>
              </div>
              <div className="space-y-2 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <a
                      href={`https://sepolia.arbiscan.io/tx/${gameState.vrfDetails.transactionHash}`}
                      target="_blank"
            ...
            {/* Initia Game Log Transaction */}
            {gameState.initiaTxHash && (
              <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-semibold text-green-700">INITIA TESTNET — GAME LOG</span>
                </div>
                <div className="space-y-2 text-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">Transaction Hash:</span>
                    <div className="flex items-center gap-2">
                      <code className="bg-white px-2 py-1 rounded text-xs">
                        {gameState.initiaTxHash.slice(0, 10)}...{gameState.initiaTxHash.slice(-8)}
                      </code>
                      <a
                        href={`https://scan.testnet.initia.xyz/evm-1/evm-txs/${gameState.initiaTxHash}`}
                        target="_blank"
                        rel="noopener noreferrer"
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded text-xs text-blue-700">
            <p className="font-medium mb-1">🔒 Dual Network Verification</p>
            <p>
              Randomness generated on Arbitrum Sepolia using Pyth Entropy, game result logged on Initia EVM Testnet.
              Both transactions are permanently recorded on their respective blockchains for independent verification.
            </p>
          </div>
        </div>
      )}
