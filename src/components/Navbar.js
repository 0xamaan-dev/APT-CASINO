...
const parseAptAmount = (amount) => {
  // Use 18 decimals for CreditCoin
...
export default function Navbar() {
  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      message: 'Your INIT balance has been updated',
      isRead: false,
      time: '2 min ago'
    },
    {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      const balanceInCtc = parseFloat(userBalance || '0');
      if (balanceInCtc <= 0) {
      ...
      console.log('🔍 Withdrawal amount (INIT):', balanceInCtc);
      ...
      notification.success(`Withdrawal transaction sent! ${formatDisplayBalance(balanceInCtc)} will be transferred. TX: ${txDisplay}`);
  ...
  const handleDeposit = async () => {
    ...
    if (amount < MIN_DEPOSIT) {
      notification.error(`Minimum deposit amount is ${MIN_DEPOSIT} INIT`);
      return;
    }

    if (amount > MAX_DEPOSIT) {
      notification.error(`Maximum deposit amount is ${MAX_DEPOSIT} INIT`);
      return;
    }

    setIsDepositing(true);
    console.log('🚀 Starting deposit process for:', amount, 'INIT');
    try {
      ...
      // Check if user is on Initia EVM Testnet network
      const currentChainId = chainId;
      const expectedChainId = initiaTestnetConfig.id; // 2124225178762456

      console.log('🔍 Current chain ID:', currentChainId);
      console.log('🔍 Expected chain ID:', expectedChainId);

      if (currentChainId !== expectedChainId) {
        throw new Error(`Please switch to ${initiaTestnetConfig.name} network (Chain ID: ${expectedChainId}). Current chain: ${currentChainId}`);
      }
      ...
      console.log('📤 Sending deposit transaction to Treasury via Wagmi...');
      ...
      try {
        ...
        if (txError.code === 4001 || txError.message?.includes('User rejected')) {
          ...
          throw new Error('A request is already pending. Please check your wallet extension.');
        } else if (txError.code === -32603) {
          throw new Error('Internal error. Please try again or restart your wallet.');
      ...
      notification.success(`Successfully deposited ${amount} INIT to casino treasury! TX: ${txHash.slice(0, 10)}...`);
      ...
      console.error('❌ Deposit failed with error details:', error);
      ...
      if (error?.code === 4100) {
        errorMsg = 'DApp interaction is disabled. Please enable it in MetaMask settings or reconnect your wallet.';
      } else if (error?.code === -32603) {
        errorMsg = 'RPC connection error. Please check your network connection and try again.';
      ...
      notification.error(`Deposit failed: ${errorMsg}`);
    } finally {
                ...
                <div className="bg-gradient-to-r from-green-900/20 to-green-800/10 rounded-lg border border-green-800/30 px-3 py-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-green-300 font-medium">
                      {isLoadingBalance ? 'Loading...' : formatDisplayBalance(userBalance)}
                    </span>
                    <button
            ...
            >
              ...
              <div className="mb-6">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Transfer INIT from your wallet to house balance for gaming
                </p>
              ...
              {/* Withdraw Section */}
              <div className="mb-4">
                <h4 className="text-sm font-medium text-white mb-2">Withdraw INIT</h4>
                <button
                ...
                >
                  {isWithdrawing ? (
                    ...
                    parseFloat(userBalance || '0') > 0 ? 'Withdraw All INIT' : 'No Balance'
                  ) : 'Connect Wallet'}
                ...
                {isConnected && parseFloat(userBalance || '0') > 0 && (
                  <p className="text-xs text-gray-400 mt-1 text-center">
                    Withdraw {formatDisplayBalance(userBalance)} to your wallet
                  </p>
                )}
              </div>
