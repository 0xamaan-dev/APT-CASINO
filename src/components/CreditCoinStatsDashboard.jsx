...
/**
 * Initia Statistics Dashboard Component
 * 
 * Validates: Requirements 9.1, 9.2, 9.3, 9.4, 9.5, 9.6, 9.7
 * 
 * Features:
 * - Fetches statistics from InitiaGameLogger.getStats
 * - Displays total games logged on Initia
 * - Displays game type counts (Roulette, Mines, Wheel, Plinko)
 * - Displays total bet amounts in INIT
 * - Displays total payout amounts in INIT
 */

// Utility function to format INIT amounts
...
const CreditCoinStatsDashboard = () => {
  ...
  // Fetch statistics from Initia (Requirement 9.1, 9.2)
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        setError('Failed to load statistics from Initia');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    ...
    >
      ...
      <Grid container spacing={2}>
        ...
        {/* Total Bets in INIT (Requirement 9.3, 9.5) */}
        <Grid item xs={12} sm={6} md={3}>
          <Box
            sx={{
              p: 2,
          ...
          >
            ...
            <Typography variant="h4" fontWeight="bold" color="white">
              {formatINITAmount(parseFloat(stats.totalBets))} INIT
            </Typography>
          </Box>
        </Grid>

        {/* Total Payouts in INIT (Requirement 9.3, 9.5) */}
        <Grid item xs={12} sm={6} md={3}>
          <Box
            sx={{
              p: 2,
          ...
          >
            ...
            <Typography variant="h4" fontWeight="bold" color="white">
              {formatINITAmount(parseFloat(stats.totalPayouts))} INIT
            </Typography>
          </Box>
        </Grid>
