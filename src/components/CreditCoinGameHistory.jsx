...
/**
 * Initia Game History Component
 * 
 * Validates: Requirements 4.9, 4.10, 9.1, 9.2, 9.8, 9.9, 9.10, 3.1, 3.2, 3.3, 3.4, 3.5, 8.4, 8.5
 * 
 * Features:
 * - Fetches game history from InitiaGameLogger contract
 * - Displays game history with Initia transaction links
 * - Shows bet amounts and payouts in INIT denomination
 * - Supports filtering by game type
 * - Displays game timestamps from Initia block data
...
const InitiaGameHistory = ({ gameType = null }) => {
  ...
  // Fetch game history from Initia (Requirement 4.9)
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        setError('Failed to load game history from Initia');
      } finally {
        setLoading(false);
      }
    };
  ...
  // Format timestamp from Initia block data (Requirement 9.10)
  ...
  // Open Initia Explorer link for transaction hash (Requirement 4.10)
  const openInitiaExplorer = (txHash) => {
    if (txHash) {
      const explorerUrl = getInitiaExplorerUrl(txHash);
  ...
  const openArbitrumExplorer = (txHash) => {
    if (txHash) {
      const explorerUrl = getPythEntropyTxExplorerUrl(txHash);
      window.open(explorerUrl, '_blank');
    }
  };
                      ...
                      <TableRow>
                        <TableCell colSpan={6} align="center" sx={{ py: 4 }}>
                          <Typography color="rgba(255,255,255,0.6)">
                            {showOnlyNFTs ? 'No games with minted NFTs found' : 'No game history found on Initia'}
                          </Typography>
                        </TableCell>
                      </TableRow>
                    ) : (
                        ...
                        >
                          ...
                          <TableCell align="center">
                            <Typography variant="body2">
                              {formatINITAmount(game.betAmount)} INIT
                            </Typography>
                          </TableCell>
                          <TableCell align="center">
                            ...
                            >
                              {parseFloat(game.payout) > 0 ? (
                                <>
                                  ...
                                  {formatINITAmount(game.payout)} INIT
                                </>
                              ) : '-'}
                            </Typography>
                          </TableCell>
                          <TableCell align="center">
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, alignItems: 'center' }}>
                              <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', justifyContent: 'center' }}>
                                {/* Initia Transaction Link (Requirement 4.10) */}
                                {game.initiaTxHash && (
                                  <Box
                                ...
                                {game.nftTxHash ? (
                                  <Box
                                    onClick={() => openInitiaExplorer(game.nftTxHash)}
                                    sx={{
                                      display: 'flex',
                                      alignItems: 'center',
                                      gap: 0.5,
                                    ...
                                    title="View NFT on Initia Explorer"
                                  >
                ...
                {stats.biggestWin && (
                  ...
                  >
                    ...
                    <Typography variant="body1" color="rgba(255,255,255,0.8)">
                      {formatINITAmount(stats.biggestWin.payout)} INIT in {GAME_TYPES[stats.biggestWin.gameType] || stats.biggestWin.gameType}
                    </Typography>
                  </Box>
                )}
              </Box>
...
export default InitiaGameHistory;
