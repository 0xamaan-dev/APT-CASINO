...
const GameHistory = ({ gameHistory }) => {
  ...
  // Open Initia EVM Testnet Explorer link
  const openInitiaExplorer = (txHash) => {
    if (txHash && txHash !== 'unknown') {
      const explorerUrl = `https://scan.testnet.initia.xyz/evm-1/evm-txs/${txHash}`;
      window.open(explorerUrl, '_blank');
    }
  };
          ...
          <table className="w-full mt-4 text-md">
            <thead className=" text-left">
              <tr>
                ...
                <th className="py-6 px-4 font-medium">NFT</th>
                  ...
                  >
                    ...
                    <td className="py-6 px-4">
                      {item.nftTxHash ? (
                        <div className="flex items-center gap-2">
                          {item.nftImagePath && !imageErrors[item.id] ? (
                            <Image
                              ...
                              loading="lazy"
                              className="rounded"
                              onError={() => handleImageError(item.id, item.nftImagePath)}
                            />
                          ) : item.nftImagePath && imageErrors[item.id] ? (
                            <div 
                              className="w-6 h-6 flex items-center justify-center bg-[#681DDB]/10 border border-[#681DDB]/30 rounded"
                          ...
                          <button
                            onClick={() => openInitiaExplorer(item.nftTxHash)}
                        ...
                        <span className="text-purple-400 text-xs">Minting...</span>
                      ) : item.nftError ? (
                        <span className="text-red-400 text-xs">Mint Failed</span>
                      ) : null}
                    </td>
                    <td className="py-6 px-4">
                      {item.entropyProof || item.initiaTxHash ? (
                        <div className="text-xs text-gray-300 font-mono">
                          ...
                          <div className="flex gap-1 mt-1 flex-wrap">
                            {item.initiaTxHash && (
                              <button
                                onClick={() => openInitiaExplorer(item.initiaTxHash)}
                                ...
                                title="View on Initia EVM Testnet Explorer"
                              >
