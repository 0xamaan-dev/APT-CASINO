...
const BettingHistory = ({ history }) => {
  const openCreditCoinExplorer = (txHash) => {
    if (txHash) {
      const network = process.env.NEXT_PUBLIC_NETWORK || 'initia-testnet';
      let explorerUrl;
      
      if (network === 'initia-testnet') {
        explorerUrl = `https://scan.testnet.initia.xyz/evm-1/evm-txs/${txHash}`;
      } else {
        explorerUrl = `https://scan.testnet.initia.xyz/evm-1/evm-txs/${txHash}`;
      }
            ...
            <Box sx={{ textAlign: 'right' }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', mt: 0.5 }}>
                ...
                <Tooltip title={bet.vrfDetails?.transactionHash ? 'VRF Verified - Click to view' : 'Result'}>
                  <Box
                    ...
                    onClick={() => bet.vrfDetails?.transactionHash && openCreditCoinExplorer(bet.vrfDetails.transactionHash)}
                  >
                    {bet.roll}
              ...
              {bet.vrfDetails?.transactionHash && (
                <Box sx={{ mt: 1, display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                  ...
                  <Tooltip title="Click to verify on Initia EVM Testnet Explorer">
                    <Chip
                      ...
                      color="success"
                      icon={<Verified />}
                      onClick={() => openCreditCoinExplorer(bet.vrfDetails.transactionHash)}
                      sx={{ 
                        fontSize: '0.6rem', 
                        height: 20,
                        cursor: 'pointer',
