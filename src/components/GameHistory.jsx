...
const GameHistory = ({ hotNumbers, coldNumbers, recentGames = [] }) => {
  ...
  const openExplorer = (txHash, network = 'Initia') => {
    if (txHash) {
      let explorerUrl;
      
      if (network === 'arbitrum-sepolia') {
        explorerUrl = getPythEntropyTxExplorerUrl(txHash);
      } else {
        explorerUrl = getInitiaExplorerUrl(txHash);
      }
  ...
  const NumberBall = ({ number, size = 32, opacity = 1, game = null }) => (
    <Tooltip 
      title={
        game?.vrfDetails?.transactionHash || game?.initiaTxHash ? (
          <Box>
            {game?.initiaTxHash && (
              <Typography variant="caption" display="block">
                Initia TX: {game.initiaTxHash.slice(0, 10)}...
              </Typography>
            )}
            {game?.vrfDetails?.transactionHash && (
              <Typography variant="caption" display="block">
                Arbitrum Sepolia Entropy TX: {game.vrfDetails.transactionHash.slice(0, 10)}...
              </Typography>
            )}
            {game?.vrfDetails?.blockNumber && (
              <Typography variant="caption" display="block">
                Block #{game.vrfDetails.blockNumber}
              </Typography>
            )}
          </Box>
        ) : 'Number result'
    ...
    >
      <Box 
        sx={{ 
          ...
          cursor: (game?.vrfDetails?.transactionHash || game?.initiaTxHash) ? 'pointer' : 'default',
          '&:hover': {
            transform: (game?.vrfDetails?.transactionHash || game?.initiaTxHash) ? 'scale(1.1)' : 'scale(1.05)',
          }
        }}
        onClick={() => {
          if (game?.initiaTxHash) {
            openExplorer(game.initiaTxHash, 'Initia');
          } else if (game?.vrfDetails?.transactionHash) {
            openExplorer(game.vrfDetails.transactionHash, 'arbitrum-sepolia');
          }
        }}
      >
        {number}
  ...
  return (
    ...
    >
      ...
      <Typography variant="body1" paragraph color="white">
        Track the results of previous spins and identify patterns. All results are verified using Pyth Entropy on Arbitrum Sepolia and logged on Initia EVM Testnet.
