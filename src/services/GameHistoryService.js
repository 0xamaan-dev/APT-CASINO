...
export class GameHistoryService {
  ...
  async getVRFDetails(vrfRequestId) {
    ...
    try {
      ...
      return {
        ...
        etherscanUrl: `${entropyTxExplorerBase}/tx/${vrf.transaction_hash}`,
        network: 'pyth-entropy',
        verifiable: true,
        verificationNote: 'Entropy from Pyth',
      };
  ...
  async getUserHistory(userAddress, options = {}) {
    ...
    try {
      ...
      const initiaExplorerBase =
        process.env.NEXT_PUBLIC_INITIA_TESTNET_EXPLORER || 'https://scan.testnet.initia.xyz/evm-1';
      const entropyTxExplorerBase =
        process.env.PYTH_ENTROPY_TX_EXPLORER_BASE || 'https://sepolia.arbiscan.io';
      ...
      const games = result.rows.map(row => {
        const game = {
          ...
          network: row.network || 'initia-testnet'
        };
        ...
        if (row.somnia_tx_hash) {
          game.somniaTransaction = {
            ...
            explorerUrl: `${initiaExplorerBase}/evm-txs/${row.somnia_tx_hash}`,
            network: 'initia-testnet',
            verificationNote: 'Game result on Initia EVM Testnet — click to verify',
          };
        }
        ...
        if (includeVrfDetails && row.entropy_tx_hash) {
          game.vrfDetails = {
            ...
            etherscanUrl: `${entropyTxExplorerBase}/tx/${row.entropy_tx_hash}`,
            network: 'pyth-entropy',
            verifiable: true,
            verificationNote: 'Entropy from Pyth — click to verify',
          };
        }
    ...
    try {
      ...
      return result.rows.map(row => ({
        ...
        somniaExplorerUrl: row.somnia_tx_hash
          ? `${initiaExplorerBase}/evm-txs/${row.somnia_tx_hash}`
          : null,
        entropyExplorerUrl: row.entropy_tx_hash
          ? `${entropyTxExplorerBase}/tx/${row.entropy_tx_hash}`
          : null,
      }));
