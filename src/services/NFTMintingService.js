 ...
 * NFT Minting Service
 * 
 * Orchestrates NFT minting after game completion on Initia EVM Testnet.
 * Operates independently from game logging to ensure game functionality is never disrupted.
 * 
 * Architecture:
 * - Event-driven: Listens to GameResultLogged events from InitiaGameLogger
...
export class NFTMintingService {
   ...
   * @param {boolean} processPastEvents - Whether to process past events (default: true)
   * @param {number} fromBlock - Block number to start processing from (default: last 1000 blocks)
   */
  async startListening(gameLoggerContract, processPastEvents = true, fromBlock = null) {
    if (!this.nftContract) {
    ...
    try {
      // Process past events if requested
      if (processPastEvents) {
        await this.processPastEvents(fromBlock);
      }
  ...
  /**
   * Process past GameResultLogged events that don't have NFTs minted yet
   * @param {number} fromBlock - Block number to start from (default: last 1000 blocks)
   */
  async processPastEvents(fromBlock = null) {
    if (!this.gameLoggerContract || !this.nftContract) {
      console.warn('⚠️ Contracts not available for past event processing');
      return;
    }
    ...
    try {
      ...
      console.log(`🔍 Processing past events from block ${startBlock} to ${currentBlock}...`);

      // Query past GameResultLogged events
      const filter = this.gameLoggerContract.filters.GameResultLogged();
      const events = await this.gameLoggerContract.queryFilter(filter, startBlock, currentBlock);

      console.log(`📋 Found ${events.length} past game events`);
      ...
      for (const event of events) {
        try {
          const { logId, player, gameType, betAmount, payout, timestamp } = event.args;

          // Check if NFT already minted for this logId
          const gameLog = await this.gameLoggerContract.getGameLog(logId);

          if (gameLog.nftTokenId > 0) {
            console.log(`⏭️  Skipping ${logId} - NFT already minted (Token #${gameLog.nftTokenId})`);
            continue;
          }

          console.log(`📝 Queuing mint for past event: ${logId}`);
          ...
          console.error(`❌ Error processing past event:`, eventError.message);
          // Continue with next event
        }
      }

      console.log(`✅ Finished processing past events`);
    } catch (error) {
      console.error('❌ Failed to process past events:', error);
      // Don't throw - service can still work for new events
    }
  }
  ...
  /**
   * Select an NFT image based on game type
   * @param {number|string} gameType - Game type ID or name
   */
  getNFTImageForGameType(gameType) {
    const images = ['/nft/nft.png', '/nft/nft1.png', '/nft/nft2.png', '/nft/nft3.png'];

    // Map game type to index (0-3)
    let index = 0;
    if (typeof gameType === 'number' || typeof gameType === 'bigint') {
      index = Number(gameType) % images.length;
    } else if (typeof gameType === 'string') {
      const type = gameType.toUpperCase();
      if (type === 'ROULETTE') index = 0;
      else if (type.includes('MINES')) index = 1;
      else if (type.includes('WHEEL')) index = 2;
      else if (type.includes('PLINKO')) index = 3;
    }
  ...
  async mintNFT(mintData) {
    ...
    const nftImagePath = this.getNFTImageForGameType(mintData.gameType);
