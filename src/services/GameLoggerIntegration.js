import { initiaGameLogger } from './CreditCoinGameLogger';
 ...
 */
export async function logGameToInitia({
  gameType,
  playerAddress,
  betAmount,
  result,
  ...
  try {
    ...
    console.log('✅ Game logged to Initia:', {
      gameType,
      txHash: data.txHash,
    ...
    console.error('❌ Failed to log game to Initia:', error);
...
/**
 * Get player's game history from Initia
...
export async function getPlayerGameHistory(playerAddress, limit = 50, provider = null) {
  try {
    ...
    if (provider) {
      initiaGameLogger.setProviderAndSigner(provider, null);
    }

    const history = await initiaGameLogger.getGameHistory(playerAddress, limit);
...
export async function getPlayerGameCount(playerAddress, provider = null) {
  try {
    ...
    const count = await initiaGameLogger.getPlayerGameCount(playerAddress);
...
export async function getGameLoggerStats(provider = null) {
  try {
    ...
    const stats = await initiaGameLogger.getStats();
...
export function subscribeToGameResults(callback, provider = null) {
  try {
    ...
    return initiaGameLogger.onGameResultLogged(callback);
...
export function getTransactionExplorerUrl(txHash) {
  return initiaGameLogger.getTransactionUrl(txHash);
}
