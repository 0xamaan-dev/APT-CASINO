...
/**
 * Pyth Entropy V2 — server-side only (rollup RPC via PYTH_ENTROPY_RPC_URL).
 * Initia handles gameplay, deposits, and logs; this route only submits entropy requests.
 */
const PYTH_ENTROPY_ADDRESS = process.env.NEXT_PUBLIC_PYTH_ENTROPY_CONTRACT || '0x549ebba8036ab746611b4ffa1423eb0a4df61440';
const PYTH_ENTROPY_RPC =
  process.env.PYTH_ENTROPY_RPC_URL ||
  process.env.NEXT_PUBLIC_PYTH_ENTROPY_RPC_URL ||
  'https://sepolia-rollup.arbitrum.io/rpc';
const PYTH_ENTROPY_CHAIN_ID = 421614;

// Minimal ABI for Pyth Entropy V2
...
export async function POST(request) {
  try {
    ...
    const provider = new ethers.JsonRpcProvider(PYTH_ENTROPY_RPC);

    const network = await provider.getNetwork();
    console.log('🌐 Connected to network:', network.name, 'Chain ID:', network.chainId.toString());

    if (Number(network.chainId) !== PYTH_ENTROPY_CHAIN_ID) {
      throw new Error(`Wrong entropy RPC chain. Expected ${PYTH_ENTROPY_CHAIN_ID}, got ${network.chainId}`);
    }

    // Check if contract exists at this address
    const code = await provider.getCode(PYTH_ENTROPY_ADDRESS);
    if (code === '0x') {
      throw new Error(`No contract found at address ${PYTH_ENTROPY_ADDRESS} on entropy RPC chain`);
    }
    ...
    const privateKey =
      process.env.PYTH_ENTROPY_SIGNER_PRIVATE_KEY ||
      process.env.ARBITRUM_TREASURY_PRIVATE_KEY ||
      process.env.TREASURY_PRIVATE_KEY;
    if (!privateKey) {
      throw new Error('PYTH_ENTROPY_SIGNER_PRIVATE_KEY (or legacy ARBITRUM_TREASURY_PRIVATE_KEY) is required');
    }
    ...
    if (balance < fee) {
      throw new Error(`Insufficient balance on entropy signer wallet ${wallet.address}`);
    }
