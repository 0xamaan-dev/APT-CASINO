...
import { FaHistory, FaChartLine, FaFire, FaExclamationCircle, FaCoins, FaInfoCircle, FaTrophy, FaDice, FaExternalLinkAlt, FaImage } from 'react-icons/fa';
import Image from 'next/image';

// Utility function to format INIT amounts with proper decimal precision
const formatINITAmount = (amount) => {
...
const RouletteHistory = ({ bettingHistory = [] }) => {
  ...
  // Open Initia EVM Testnet Explorer link for transaction hash
  const openInitiaExplorer = (hash) => {
    if (hash && hash !== 'unknown') {
      const explorerUrl = `https://scan.testnet.initia.xyz/evm-1/evm-txs/${hash}`;
  ...
  const handleImageError = (betId, imagePath) => {
    console.error('Failed to load NFT image:', imagePath);
    setImageErrors(prev => ({ ...prev, [betId]: true }));
  };
                  ...
                  <TableBody>
                    {bets.map((bet) => (
                      ...
                      >
                        ...
                        <TableCell align="center">{formatINITAmount(bet.amount || bet.totalBetAmount || 0)} INIT</TableCell>
                          ...
                          >
                            {bet.win ? (
                              <>
                                ...
                                +{formatINITAmount(bet.payout || bet.netResult || 0)} INIT
                              </>
                            ) : '-'}
                          </Typography>
                        </TableCell>
                        <TableCell align="center">
                          {hasTxHash(bet.nftTxHash) ? (
                            <a
                              href={`https://scan.testnet.initia.xyz/evm-1/evm-txs/${bet.nftTxHash}`}
                              target="_blank"
                              rel="noopener noreferrer"
                              style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 6, backgroundColor: 'rgba(104,29,219,0.15)', border: '1px solid rgba(104,29,219,0.4)', color: '#681DDB', fontWeight: 'bold', fontSize: '0.75rem', textDecoration: 'none' }}
                              title="View NFT mint transaction on Initia Explorer"
                            >
                              <FaImage size={12} color="#681DDB" /> NFT
                            </a>
                          ) : (
                            <Typography variant="caption" color="rgba(255,255,255,0.45)">Pending</Typography>
                          )}
                        </TableCell>
                        <TableCell align="center">
                          <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', justifyContent: 'center' }}>
                            {hasTxHash(bet.initiaTxHash) ? (
                              <a
                                href={`https://scan.testnet.initia.xyz/evm-1/evm-txs/${bet.initiaTxHash}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 6px', borderRadius: 4, backgroundColor: 'rgba(20,216,84,0.1)', border: '1px solid rgba(20,216,84,0.3)', color: '#14D854', fontSize: '0.7rem', fontWeight: 'bold', textDecoration: 'none' }}
                                title="View on Initia Explorer"
                              >
                                <FaExternalLinkAlt size={10} color="#14D854" /> Initia
                              </a>
                            ) : (
                              <Typography variant="caption" color="rgba(255,255,255,0.45)">Initia pending</Typography>
                            )}
                            {hasTxHash(bet.entropyTxHash || bet.entropyProof?.transactionHash) && (
                              <a
                                href={`https://entropy-explorer.pyth.network/?chain=arbitrum-sepolia&search=${bet.entropyTxHash || bet.entropyProof?.transactionHash}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 6px', borderRadius: 4, backgroundColor: 'rgba(104,29,219,0.1)', border: '1px solid rgba(104,29,219,0.3)', color: '#681DDB', fontSize: '0.7rem', fontWeight: 'bold', textDecoration: 'none' }}
                                title="View on Pyth Entropy Explorer"
                              >
                                <FaExternalLinkAlt size={10} color="#681DDB" /> Entropy
                              </a>
                            )}
                          </Box>
                        </TableCell>
              ...
              <Box>
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mb: 4 }}>
                  ...
                  >
                    ...
                    <Typography variant="h4" fontWeight="bold" color="white" sx={{ textShadow: '0 2px 4px rgba(0,0,0,0.5)' }}>{formatINITAmount(stats.totalWagered)} INIT</Typography>
                  </Box>
                  ...
                  >
                    ...
                    >
                      {stats.netProfit >= 0 ? '+' : ''}{formatINITAmount(stats.netProfit)} INIT
                    </Typography>
                  </Box>
                </Box>
