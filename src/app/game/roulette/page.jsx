"use client";
// Updated for Creditcoin migration - v2.3 (File refresh - 2026-03-07)
...
import { BetBox, GridInside, GridZero, GridColumnBet, GridOutsideBet } from './components/BettingComponents';
...
// BetBox component extracted to ./components/BettingComponents.jsx

// Betting grid components extracted to ./components/BettingComponents.jsx
...
// RouletteWheel extracted to ./components/RouletteWheel.jsx

// BettingStats extracted to ./components/BettingStats.jsx
...
export default function GameRoulette() {
              ...
              <div className="bg-gradient-to-br from-red-900/20 to-red-800/5 rounded-xl p-4 border border-red-800/20 shadow-lg shadow-red-900/10">
                ...
                >
                  ...
                  <div className="flex flex-col items-center p-2 bg-black/20 rounded-lg min-w-0 w-full">
                    ...
                    <div className="text-white font-display text-sm md:text-base truncate w-full text-center" title={`${gameStatistics.totalVolume} INIT`}>
                      {gameStatistics.totalVolume} INIT
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center p-2 bg-black/20 rounded-lg min-w-0 w-full">
                    ...
                    <div className="text-white font-display text-sm md:text-base truncate w-full text-center" title={`${gameStatistics.maxWin} INIT`}>
                      {gameStatistics.maxWin} INIT
                    </div>
                  </div>
                </motion.div>
  ...
  const mergeHistoryEntries = (currentEntries, fetchedEntries) => {
    const fetchedKeys = new Set(
      fetchedEntries
        .map((entry) => entry.initiaTxHash || entry.logId || entry.id)
        .filter(Boolean)
    );

    const pendingEntries = currentEntries.filter((entry) => {
      const key = entry.initiaTxHash || entry.logId || entry.id;
      return !key || !fetchedKeys.has(key);
    });

    return [...pendingEntries, ...fetchedEntries].sort((a, b) => {
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      ...
      try {
        const logs = await getHistory(50);
        const normalizedHistory = logs
          .filter((game) => game.gameType === 'ROULETTE')
          .map((game) => ({
            ...game,
            id: game.logId || game.id,
            betType: game.betType || game.title || 'Roulette',
            amount: Number(game.amount ?? game.betAmount ?? 0),
            result:
              game.winningNumber ??
              game.result?.winningNumber ??
              game.result?.result ??
              null,
            payout: Number(game.payoutAmount ?? game.payout ?? 0),
            win: Boolean(game.win),
            time: game.time,
            timestamp: game.timestamp,
            initiaTxHash: game.initiaTxHash || null,
            nftTxHash: game.nftTxHash || null,
            nftTokenId: game.nftTokenId || null,
            nftImagePath: game.nftImagePath || null,
            entropyTxHash: game.entropyTxHash || null,
            entropyProof: game.entropyTxHash
              ? { transactionHash: game.entropyTxHash }
              : null,
          }));

        if (!cancelled) {
          setBettingHistory((currentHistory) =>
            mergeHistoryEntries(currentHistory, normalizedHistory)
          );
        }
      } catch (error) {
        console.warn('Failed to hydrate Roulette history:', error);
      }
    };
  ...
  }, [address, getHistory, isConnected]);

  // Function to fetch real INIT balance will be defined after useSelector
  ...
  // Function to fetch real INIT balance
  ...
  const lockBet = async () => {
    ...
    const currentBalance = parseFloat(userBalance || '0'); // Balance is already in INIT
    const totalBetAmount = total;

    if (currentBalance < totalBetAmount) {
      alert(`Insufficient balance. You have ${currentBalance.toFixed(5)} INIT but need ${totalBetAmount.toFixed(5)} INIT`);
      return;
    }
    ...
    try {
      ...
      if (originalBalance < totalBetAmount) {
        alert(`Insufficient balance. You have ${originalBalance.toFixed(5)} INIT but need ${totalBetAmount.toFixed(5)} INIT`);
      ...
      setTimeout(() => {
        ...
        pythEntropyService.generateRandom('ROULETTE', {
          ...
          // Log game result to Initia EVM Testnet (non-blocking)
          logGame({
            gameType: 'ROULETTE',
            ...
            if (txHash) {
              console.log('✅ Roulette game logged to Initia:', getExplorerUrl(txHash));
              setBettingHistory(prev => {
                const updatedHistory = [...prev];
                if (updatedHistory.length > 0) {
                  updatedHistory[0] = { ...updatedHistory[0], initiaTxHash: txHash };
                }
                return updatedHistory;
              });
            }
          }).catch(error => {
            console.warn('⚠️ Failed to log Roulette game to Initia:', error);
          });
        ...
        if (netResult > 0) {
          const winMessage = winningBets.length === 1
            ? `🎉 WINNER! ${winningBets[0].name} - You won ${(netResult - totalBetAmount).toFixed(5)} INIT!`
            : `🎉 MULTIPLE WINNERS! ${winningBets.length} bets won - Total: ${(netResult - totalBetAmount).toFixed(5)} INIT!`;

          setNotificationMessage(winMessage);
          setNotificationSeverity("success");
          setSnackbarMessage(winMessage);
        } else {
          setNotificationMessage(`💸 Number ${winningNumber} - You lost ${totalBetAmount.toFixed(5)} INIT!`);
          setNotificationSeverity("error");
          setSnackbarMessage(`💸 Number ${winningNumber} - You lost ${totalBetAmount.toFixed(5)} INIT!`);
        }
      ...
      const checkWithRetry = async (attempts = 3) => {
        if (window.ethereum && typeof window.ethereum.request === 'function') {
          ...
          try {
            ...
            // Support Initia EVM Testnet (0x18e8f)
            const isCorrectNetwork = chainId === "0x18e8f" || chainId === "2124225178762456";
  ...
  const switchNetwork = async () => {
    ...
    try {
      ...
      console.log("Attempting to switch to Initia EVM Testnet network");

      try {
        // Try Initia EVM Testnet first
        await window.ethereum.request({
          method: "wallet_switchEthereumChain",
          params: [{ chainId: "0x18e8f" }],
        });

        console.log("Successfully switched to Initia EVM Testnet");
        ...
        if (switchError.code === 4902) {
          try {
            console.log("Adding Initia EVM Testnet to wallet");
            await window.ethereum.request({
            ...
            try {
              ...
              console.log("Successfully switched to Initia EVM Testnet after adding");
              // Set network to correct and reload after short delay
              setCorrectNetwork(true);
              setTimeout(() => window.location.reload(), 1000);
              return;
            } catch (error) {
              console.error("Error switching to Initia after adding:", error);
            }
          } catch (addError) {
            console.error("Failed to add Initia EVM Testnet:", addError);
            alert("Unable to switch to required network. Please try adding Initia EVM Testnet manually.");
          }
        } else {
          // Handle other errors
          console.error("Failed to switch network:", switchError);
          alert("Failed to switch network. Please try again or add Initia EVM Testnet manually.");
        }
      }
    } catch (error) {
      ...
      <div ref={contentRef} className="font-sans" style={{ backgroundColor: "#080005", minHeight: "100vh", overflowX: 'hidden', paddingTop: "30px" }}>
        ...
        >
          ...
          >
            ...
            >
              <FaCoins className="text-yellow-400" />
              Balance: {isConnected ? `${parseFloat(userBalance || '0').toFixed(5)} INIT` : 'Connect Wallet'}
            </Typography>
          </Box>
          ...
          >
            ...
            <Box sx={{
              ...
              <Typography color="white" sx={{ opacity: 0.8 }}>
                Current Bet Total: {total.toFixed(5)} INIT
              </Typography>
                ...
                {rollResult >= 0 ? (
                  ...
                  <Box sx={{ display: "flex", flexDirection: "column" }}>
                    <Button
                      ...
                      onClick={lockBet}
                    >
                      {total > 0 ? `Place Bet (${total.toFixed(5)} INIT)` : 'Place Bet (INIT)'}
                    </Button>
        ...
        >
          ...
          >
            ...
            {notificationIndex === notificationSteps.RESULT_READY && (
              <Typography>
                {winnings > 0
                  ? `🎉 You won ${winnings.toFixed(4)} INIT!`
                  : winnings < 0
                    ? `💸 You lost ${Math.abs(winnings).toFixed(4)} INIT!`
                    : "🤝 Break even!"}
              </Typography>
            )}
          </MuiAlert>
