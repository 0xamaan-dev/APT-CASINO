...
export default function Home() {
  ...
  // CreditCoin logging state
  const [isCreditCoinLogging, setIsCreditCoinLogging] = useState(false);
  const [creditCoinError, setCreditCoinError] = useState(null);
    ...
    const loadHistory = async () => {
      ...
      try {
        ...
        const normalizedHistory = logs
          .filter((game) => game.gameType === 'WHEEL')
          .map((game) => ({
            ...game,
            ...
            multiplier: game.multiplier || '0.00x',
        ...
        console.warn('Failed to hydrate Wheel history:', error);
      }
    };
  ...
  const manulBet = async () => {
    ...
    const generateEntropyInBackground = async (historyItemId, winAmount, actualMultiplier, currentZetaEnabled, currentAddress, currentIsConnected) => {
      try {
        ...
        logGame({
          ...
          if (txHash) {
            console.log('✅ Wheel game logged to Initia:', getExplorerUrl(txHash));
            // Update game history with Creditcoin transaction hash
            setGameHistory(prev => prev.map(item =>
              item.id === historyItemId
                ? { ...item, initiaTxHash: txHash }
                : item
            ));
          }
        }).catch(error => {
          console.warn('⚠️ Failed to log Wheel game to Initia:', error);
        });
    ...
    // Check Redux balance (balance is already in INIT)
    const currentBalance = parseFloat(userBalance || '0');

    if (currentBalance < betAmount) {
      alert(`Insufficient balance. You have ${currentBalance.toFixed(5)} INIT but need ${betAmount} INIT`);
      return;
    }
      ...
      window.wheelBetCallback = async (landedMultiplier) => {
        ...
        setTimeout(async () => {
          ...
          if (actualMultiplier > 0) {
            notification.success(`Congratulations! ${betAmount} INIT × ${actualMultiplier.toFixed(2)} = ${winAmount.toFixed(5)} INIT won!`);
  ...
  const autoBet = async ({
    ...
    for (let i = 0; i < numberOfBets; i++) {
      ...
      if (currentBalance < currentBet) {
        alert(`Insufficient balance for bet ${i + 1}. Need ${currentBet.toFixed(5)} INIT but have ${currentBalance.toFixed(5)} INIT`);
        break;
      }
      ...
      if (actualMultiplier > 0) {
        notification.success(`Congratulations! ${currentBet} INIT × ${actualMultiplier.toFixed(2)} = ${winAmount.toFixed(8)} INIT won!`);
      }
      ...
      <div className="px-4 md:px-8 lg:px-20">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="w-full lg:w-1/3">
            ...
            {/* Creditcoin logging status indicator */}
            {isLogging && (
