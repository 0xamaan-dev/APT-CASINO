...
import { FaHistory, FaFilter, FaDownload, FaSearch, FaTrophy, FaChartLine, FaExternalLinkAlt, FaCheck, FaImage } from "react-icons/fa";
            ...
            {paginatedHistory.map((item, index) => (
              ...
              >
                ...
                >
                  ...
                  >
                    {hasTxHash(item.nftTxHash) ? (
                      <a
                        href={`https://scan.testnet.initia.xyz/evm-1/evm-txs/${item.nftTxHash}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 6, backgroundColor: 'rgba(104,29,219,0.15)', border: '1px solid rgba(104,29,219,0.4)', color: '#681DDB', textDecoration: 'none', fontSize: '0.75rem', fontWeight: 'bold' }}
                        title="View NFT mint transaction on Initia Explorer"
                      >
                        <FaImage size={12} color="#681DDB" />
                        NFT
                      </a>
                    ) : (
                  ...
                  >
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                        {hasTxHash(item.initiaTxHash) ? (
                          <a
                            href={`https://scan.testnet.initia.xyz/evm-1/evm-txs/${item.initiaTxHash}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: '#14D854', fontSize: '0.7rem', textDecoration: 'none' }}
                            title="View on Initia Explorer"
                          >
                            <FaExternalLinkAlt size={10} /> Initia
                          </a>
                        ) : (
                          <Typography variant="caption" color="rgba(255,255,255,0.45)">Initia pending</Typography>
                        )}
                        {hasTxHash(item.entropyTxHash || item.entropyProof?.transactionHash) && (
                          <a
                            href={`https://entropy-explorer.pyth.network/?chain=arbitrum-sepolia&search=${item.entropyTxHash || item.entropyProof?.transactionHash}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: '#681DDB', fontSize: '0.7rem', textDecoration: 'none' }}
                            title="View on Pyth Entropy Explorer"
                          >
                            <FaExternalLinkAlt size={10} /> Entropy
                          </a>
                        )}
                      </Box>
                      <Typography variant="caption" color="rgba(255,255,255,0.5)">
                        Game logged on Initia
                      </Typography>
                    </Box>
                  </TableCell>
                </TableRow>
