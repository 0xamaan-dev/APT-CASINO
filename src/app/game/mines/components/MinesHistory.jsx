...
import { FaHistory, FaStar, FaTrophy, FaChartBar, FaChartLine, FaBomb, FaSort, FaSortUp, FaSortDown, FaExternalLinkAlt, FaImage } from "react-icons/fa";
        ...
        >
          {history.map((game, index) => (
            <motion.div
              ...
              className={`grid grid-cols-8 gap-2 p-3 text-xs rounded-lg transition-all ${game.outcome === 'win'
            ...
            >
              ...
              <div className="text-white/70 flex items-center justify-center">
                {hasTxHash(game.nftTxHash) ? (
                  <a
                    href={`https://scan.testnet.initia.xyz/evm-1/evm-txs/${game.nftTxHash}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-[#681DDB]/20 border border-[#681DDB]/40 rounded-lg text-[#681DDB] text-xs font-bold hover:bg-[#681DDB]/30 transition-all transform hover:-translate-y-0.5 shadow-sm shadow-purple-900/20 no-underline"
              ...
              <div className="text-white/70 flex items-center justify-center">
                  <div className="flex flex-col gap-1 items-center">
                    {game.entropyProof?.sequenceNumber && game.entropyProof.sequenceNumber !== '0' && (
                      <div className="text-yellow-400 font-bold text-xs font-mono">{String(game.entropyProof.sequenceNumber)}</div>
                    )}
                    <div className="flex gap-1 flex-wrap justify-center">
                      {hasTxHash(game.initiaTxHash) ? (
                        <a
                          href={`https://scan.testnet.initia.xyz/evm-1/evm-txs/${game.initiaTxHash}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-1 px-2 py-1 bg-[#0A0A0A]/10 border border-[#65B3AE]/30 rounded text-[#65B3AE] text-xs hover:bg-[#65B3AE]/20 transition-colors no-underline"
                      ...
                      {hasTxHash(game.entropyTxHash || game.entropyProof?.transactionHash) && (
                        <a
                          href={`https://entropy-explorer.pyth.network/?chain=arbitrum-sepolia&search=${game.entropyTxHash || game.entropyProof?.transactionHash}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-1 px-2 py-1 bg-[#681DDB]/10 border border-[#681DDB]/30 rounded text-[#681DDB] text-xs hover:bg-[#681DDB]/20 transition-colors no-underline"
