...
export default function Mines() {
    ...
    const loadHistory = async () => {
      ...
      try {
        ...
        const normalizedHistory = logs
          .filter((game) => game.gameType === 'MINES')
          .map((game) => ({
            ...game,
            id: game.logId || game.id,
            mines: game.result?.minePositions?.length || 0,
            bet: `${game.betAmount} INIT`,
            outcome: game.outcome || 'loss',
            payout: `${Number(game.payoutAmount ?? game.payout ?? 0).toFixed(5)} INIT`,
        ...
        console.warn('Failed to hydrate Mines history:', error);
      }
    };
  ...
  const handleFormSubmit = async (formData) => {
    try {
      ...
      console.log('🔗 Network: Initia EVM Testnet | Token: INIT | Protocol: Pyth Entropy');
      ...
      console.log(`🎮 Game Config: ${formData.mines || 3} mines | ${formData.betAmount || '0.01'} INIT bet`);
  ...
  const handleGameComplete = async (result) => {
    ...
    try {
      ...
      logGame({
        ...
        if (txHash) {
          console.log('✅ Mines game logged to Initia:', getExplorerUrl(txHash));
        ...
        console.warn('⚠️ Failed to log Mines game to Initia:', error);
      });
    ...
    const newHistoryItem = {
      id: Date.now(),
      mines: result.mines || 0,
      bet: `${result.betAmount || '0.00000'} INIT`,
      outcome: result.won ? 'win' : 'loss',
      payout: result.won ? `${result.payout || '0.00000'} INIT` : '0.00000 INIT',
