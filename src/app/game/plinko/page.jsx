...
export default function Plinko() {
  ...
  const { logGame, isLogging, getExplorerUrl, getLogsByGameType } = useCreditcoinGameLogger();
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      ...
      try {
        const logs = await getLogsByGameType('PLINKO', 50);
        const normalizedHistory = logs
          .filter((game) => game.player?.toLowerCase() === address.toLowerCase())
          .map((game) => ({
            ...game,
            id: game.logId || game.id,
            title: game.title || 'Plinko Game',
            betAmount: game.betAmount,
            multiplier: game.multiplier || '0.00x',
            payout: game.payoutAmount ?? game.payout ?? '0',
            time: game.time,
        ...
        console.warn('Failed to hydrate Plinko history:', error);
      }
    };
  ...
  }, [address, getLogsByGameType, isConnected]);
  ...
  const handleBet = () => {
    // Trigger the ball dropping animation in PlinkoGame. balance MOName
  ...
  const handleBetHistoryChange = async (newBetResult) => {
    ...
    try {
      ...
      logGame({
        ...
        if (txHash) {
          console.log('✅ Plinko game logged to Initia:', getExplorerUrl(txHash));
        ...
        console.warn('⚠️ Failed to log Plinko game to Initia:', error);
      });
