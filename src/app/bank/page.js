...
// Assets for borrowing on Initia EVM Testnet only
const BORROW_ASSETS = {
  initia_testnet: [
    {
      symbol: "INIT",
      name: "Initia EVM Testnet Network Coin",
      iconColor: "#000000",
...
const MOCK_TRANSACTIONS = [
  { type: 'deposit', token: 'INIT', amount: '120.5', date: new Date(Date.now() - 86400000 * 2), status: 'completed' },
  { type: 'borrow', token: 'INIT', amount: '0.3', date: new Date(Date.now() - 86400000), status: 'completed' },
  { type: 'swap', tokenFrom: 'INIT', tokenTo: 'USDC', amountFrom: '0.2', amountTo: '98.32', date: new Date(), status: 'completed' }
];

export default function Bank() {
  const [chainId, setChainId] = useState('initia_testnet'); // Default to Initia EVM Testnet
  ...
  useEffect(() => {
    ...
    if (isDev) {
      setChainId('initia_testnet'); // Initia EVM Testnet for development
      setAssets([
        {
          symbol: "INIT",
          name: "Initia EVM Testnet Network Coin",
    ...
    // Load Initia EVM Testnet data
    const loadChainData = async () => {
      try {
        // Set to Initia EVM Testnet
        setChainId('initia_testnet');

        // Set mock lending market data for Initia EVM Testnet
        setAssets([
          {
            symbol: "INIT",
            name: "Initia EVM Testnet Network Coin",
  ...
  // Get appropriate borrow assets for Initia EVM Testnet
  const borrowAssets = BORROW_ASSETS.initia_testnet;
        ...
        {showNetworkBanner && (
          <div className="bg-gradient-to-r from-red-magic/80 to-blue-magic/80 py-2 px-4 text-center relative mb-8 rounded-lg">
            <p className="text-white text-sm">
              Connected to Initia EVM Testnet.
            ...
            <>
              <div className="max-w-2xl mx-auto mb-12">
                <div className="bg-gradient-to-r p-[1px] from-red-magic to-blue-magic rounded-xl">
                  {/* Initia EVM Testnet Only - No Uniswap Integration */}
                  <div className="bg-gray-800 rounded-lg p-6 text-center">
                    <h3 className="text-xl font-semibold text-white mb-2">Initia EVM Testnet Only</h3>
                    <p className="text-gray-400">This application works exclusively with Initia EVM Testnet</p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-[#250020] p-4 rounded-lg hover:bg-[#350030] transition-colors">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-white/70 text-sm">INIT Price</span>
